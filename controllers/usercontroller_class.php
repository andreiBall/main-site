...
class UserController extends Controller {
	...
	public function actionEditProfile() {
		...
		if ($this->request->change_avatar) {//изменение аватарки
			$img = $this->fp->uploadIMG($message_avatar_name, $_FILES["avatar"], Config::MAX_SIZE_AVATAR, Config::DIR_AVATAR);
			if ($img) {
				$tmp = $this->auth_user->getAvatar();//удаление старой аватарки
		...
		elseif ($this->request->change_name) {//изменение имени
		...
		elseif ($this->request->change_password) {//изменение пароля
		...
		$form_avatar = new Form();//форма смены аватарки
		...
		$form_name = new Form();//форма смены имени
		$form_name->name = "change_name";
		$form_name->header = "<h2>Изменить имя</h2>";
	...
	/*-ограничение для страниц - только авторизованный пользователь-*/
